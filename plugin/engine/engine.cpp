 ...
 *
 * PLUGIN COPY: this file started as a verbatim copy of core/synth_core.cpp
 * (v1.0). The firmware never uses it. Here all mutable state lives in
 * Engine::Impl, so a host can run any number of engines side by side.
 */
#include <atomic>
#include <math.h>
#include "daisysp.h"
...
/* ---------------------------------------------------------- the instance */
// Every mutable piece of the engine; one per Engine. Value-initialized
// (new Impl()), so everything without an initializer starts at zero, as
// the file-level statics of the firmware core do.
constexpr int kRingMax = 16;
constexpr int kCirclePts = 256;
struct Engine::Impl
{
    // parameters
    std::atomic<float> params_[kNumFuncs][kOscs]; // engine inputs (any thread)
    float              smooth_[kNumFuncs][kOscs]; // audio-thread smoothed copy
    int                step_[kNumFuncs][kOscs];   // stepped functions, audio thread
    float              samplerate_ = 48000.f;

    // panel (UI thread)
    float            pot_[kNumPots];     // last reading, -1 = none yet
    float            potRef_[kNumPots];  // reading when the page was chosen
    bool             picked_[kNumPots];  // moved since the page was chosen
    int              page_ = -1;         // active page, -1 until pot 10 reads
    int              sub_[kNumPages];    // chosen sub-page per page
    float            subSel_[kNumPages]; // knob 10 reading that chose it
    std::atomic<int> activePot_;         // last pot that really moved
    float            lastShown_[kNumPots]; // readout deadband memory

    // voices
    Voice voices_[kOscs];
    std::atomic<float> oscNote_[kOscs]; // current pitch, for tests and colors
    ...
    int lfoDiv_[kLfoTargets][kOscs]; // TEMPO: division step (hysteresis)

    // scope and readout fade
    float            scope_[kScopeTraces][kScopeSize];
    std::atomic<int> scopeWrite_;
    std::atomic<int> moveCount_; // bumped by SetPot on real moves
    int              lastMove_   = 0;
    int              idleFrames_ = 0;
    ...
    // harmony
    int keyIv_[12]; // scale key: the scale as intervals above the tonic
    int keyN_ = 7;
    int chordDeg_ = 0;
    bool chordKey_ = false;
    int chordRootSemis_ = 0; // chord root, semitones above the tonic
    int chordIv_[12];        // chord key: the chord's intervals
    int chordN_ = 3;

    // what the screen shows (written by the audio thread)
    std::atomic<int> dispRootPc_;  // chord root pitch class
    std::atomic<int> dispDeg_;     // chord degree 0..6
    std::atomic<int> dispQual_;    // scale key: kQual*, chord key: 100 + chord index
    std::atomic<int> dispProgPos_; // -1 = PROG OFF
    std::atomic<int> dispProgLen_;
    std::atomic<int> dispBeat_;    // beat in the bar, 0..3
    std::atomic<int> dispKeyMask_;   // key notes, absolute pitch classes (bit 0 = C)
    std::atomic<int> dispChordMask_; // current chord's tones, absolute

    // step rings
    std::atomic<int>   ringLen_[kOscs]; // LENGTH, 0 = droning
    std::atomic<int>   ringPos_[kOscs]; // pattern position playing now
    int8_t             ringNote_[kOscs][kRingMax];    // MIDI note or kRing*
    uint8_t            ringRatchet_[kOscs][kRingMax]; // hit more than once
    std::atomic<float> oscHz_[kOscs];                 // sounding frequency (scope period)
    std::atomic<float> oscLvl_[kOscs];                // LEVEL gain (the rings divide it out)

    // unit circle
    float  circSin_[kCirclePts], circCos_[kCirclePts];
    bool   circReady_ = false;

    // text the UI thread hands out (were function-local statics)
    char valueText_[16];
    char chordName_[12];
    char chordNumeral_[8];
    // DrawWheel: each osc's last and previous note
    float wheelLast_[kOscs] = {-1.f, -1.f, -1.f}, wheelPrev_[kOscs] = {-1.f, -1.f, -1.f};

    void UpdateHarmony(int key, int deg);
    int DegreeSemis(int d);
    int ChordAt(double beat, int* progPos = nullptr);
    double RestartBeat(double beat);
    int TriadQuality();
    int KeyFifth(int d);
    int BuildPool(int o, int* out);
    void Init(float samplerate, const Buffers& buffers);
    void NoteMove(int pot, float v);
    int PageIndex();
    void Rearm(int count);
    int SlotCode(int pot);
    ...
    void SetPitch(Voice& vc);
    void Strike(Voice& vc, float midi, int gate);
    void ControlTick(Voice& vc, float dn, float cutOct, float fAmt, float fAttInc, float fDecSec, float pDecSec, float fcMax);
    void ProcessAudio(float* out, int nframes);
    int StepOf(int f, int o, float v);
    const char* GetValueText(int pot);
    bool DispMinor();
    const char* GetChordName();
    const char* GetChordNumeral();
    void DrawReadout(uint16_t* fb, float alpha);
    void DrawRows(uint16_t* fb, int pg, int sub, int y0, float alpha);
    void DrawPageMap(uint16_t* fb, float alpha);
    bool SubInUse(int pg, int sub);
    void DrawSubMap(uint16_t* fb, float alpha);
    void DrawTrace(uint16_t* fb, const float* ring, int yc, uint16_t c, float gainPx = kScopeGainPx);
    void DrawScope(uint16_t* fb, float alpha);
    void DrawChordCenter(uint16_t* fb, float alpha);
    void UnitCircle();
    int CircX(int i, float r);
    int CircY(int i, float r);
    void DrawRings(uint16_t* fb, float alpha, bool oneHue);
    void DrawWheel(uint16_t* fb, float alpha, float centerA);
...
void Engine::Impl::UpdateHarmony(int key, int deg)
{
    chordDeg_ = deg;
...
int Engine::Impl::DegreeSemis(int d)
{
    if(chordKey_)
...
int Engine::Impl::ChordAt(double beat, int* progPos)
{
...
double Engine::Impl::RestartBeat(double beat)
{
...
int Engine::Impl::KeyFifth(int d)
{
...
int Engine::Impl::BuildPool(int o, int* out)
{
...
/* ------------------------------------------------------------------ init */
void Engine::Impl::Init(float samplerate, const Buffers& buffers)
{
...
/* ----------------------------------------------------------------- panel */
void Engine::Impl::NoteMove(int pot, float v)
{
...
void Engine::Impl::Rearm(int count)
{
...
int Engine::Impl::SlotCode(int pot)
{
...
void Engine::Impl::SetPot(int pot, float v)
{
...
float Engine::Impl::GetPot(int pot)
{
    if(pot < 0 || pot >= kNumPots)
        return 0.f;
    if(pot_[pot] >= 0.f)
...
int Engine::Impl::GetPage()
{
    return PageIndex();
}

const char* Engine::Impl::GetPageName(int page)
{
    return page >= 0 && page < kNumPages ? kPages[page].name : "";
}

int Engine::Impl::GetNumSubs(int page)
{
    return page >= 0 && page < kNumPages ? kPages[page].numSubs : 1;
}

int Engine::Impl::GetSub(int page)
{
    return page >= 0 && page < kNumPages ? sub_[page] : 0;
}

const char* Engine::Impl::GetSubName(int page, int sub)
{
...
float Engine::Impl::GetSlotValue(int pot)
{
    if(pot == kPagePot)
...
const char* Engine::Impl::GetSlotName(int pot)
{
    if(pot == kPagePot)
        return "PAGE";
    if(pot == kVolumePot)
...
int Engine::Impl::GetSlotOsc(int pot)
{
...
void Engine::Impl::SetEngineParam(int func, int osc, float v)
{
...
float Engine::Impl::GetOscNote(int osc)
{
...
inline void Engine::Impl::SetPitch(Voice& vc)
{
...
void Engine::Impl::Strike(Voice& vc, float midi, int gate)
{
    vc.target = midi;
    if(vc.glideSec <= 0.f)
        vc.midi = midi;
...
inline void Engine::Impl::ControlTick(Voice& vc, float dn, float cutOct, float fAmt, float fAttInc,
...
void Engine::Impl::ProcessAudio(float* out, int nframes)
{
...
int Engine::Impl::StepOf(int f, int o, float v)
{
...
const char* Engine::Impl::GetValueText(int pot)
{
    char* const buf = valueText_;
    char*       p = buf;
    buf[0]        = 0;
    if(pot == kPagePot)
    {
...
const char* Engine::Impl::GetChordName()
{
    char* const buf = chordName_;
    const int   q = dispQual_.load();
...
const char* Engine::Impl::GetChordNumeral()
{
    // Roman numeral of the chord degree, lowercase for a minor chord.
    char* const buf = chordNumeral_;
...
void Engine::Impl::DrawReadout(uint16_t* fb, float alpha)
{
...
void Engine::Impl::DrawRows(uint16_t* fb, int pg, int sub, int y0, float alpha)
{
...
void Engine::Impl::DrawPageMap(uint16_t* fb, float alpha)
{
    const int pg = PageIndex();
...
bool Engine::Impl::SubInUse(int pg, int sub)
{
    if(pg != kPgLfo)
        return true;
...
void Engine::Impl::DrawSubMap(uint16_t* fb, float alpha)
{
    const int pg = PageIndex();
...
void Engine::Impl::DrawTrace(uint16_t* fb, const float* ring, int yc, uint16_t c,
                      float gainPx)
{
...
void Engine::Impl::DrawScope(uint16_t* fb, float alpha)
{
...
void Engine::Impl::DrawChordCenter(uint16_t* fb, float alpha)
{
...
inline int Engine::Impl::CircX(int i, float r)
{
    return 120 + (int)floorf(r * circSin_[i & (kCirclePts - 1)] + 0.5f);
}
inline int Engine::Impl::CircY(int i, float r)
{
 ...
 *  40 %, was too subtle.) */
void Engine::Impl::DrawRings(uint16_t* fb, float alpha, bool oneHue)
{
...
void Engine::Impl::DrawWheel(uint16_t* fb, float alpha, float centerA)
{
    UnitCircle();
    ...
    for(int o = kOscs - 1; o >= 0; o--)
    {
        ...
        if(fabsf(m - wheelLast_[o]) > 0.5f)
        {
            wheelPrev_[o] = wheelLast_[o];
            wheelLast_[o] = m;
        }
        for(int t = 0; t < 2; t++)
        {
            const float n = t == 0 ? wheelPrev_[o] : m;
            if(n < 0.f)
                continue;
...
void Engine::Impl::RenderDiagnostics(uint16_t* fb)
{
...
void Engine::Impl::RenderScreen(uint16_t* fb)
{
...
void Engine::Init(float samplerate, const Buffers& buffers)
{
    impl_->Init(samplerate, buffers);
}
void Engine::SetPot(int pot, float value01)
{
    impl_->SetPot(pot, value01);
}
float Engine::GetPot(int pot)
{
    return impl_->GetPot(pot);
}
int Engine::GetPage()
{
    return impl_->GetPage();
}
const char* Engine::GetPageName(int page)
{
    return impl_->GetPageName(page);
}
int Engine::GetNumSubs(int page)
{
    return impl_->GetNumSubs(page);
}
int Engine::GetSub(int page)
{
    return impl_->GetSub(page);
}
const char* Engine::GetSubName(int page, int sub)
{
    return impl_->GetSubName(page, sub);
}
float Engine::GetSlotValue(int pot)
{
    return impl_->GetSlotValue(pot);
}
const char* Engine::GetSlotName(int pot)
{
    return impl_->GetSlotName(pot);
}
int Engine::GetSlotOsc(int pot)
{
    return impl_->GetSlotOsc(pot);
}
const char* Engine::GetValueText(int pot)
{
    return impl_->GetValueText(pot);
}
void Engine::SetEngineParam(int func, int osc, float value01)
{
    impl_->SetEngineParam(func, osc, value01);
}
float Engine::GetOscNote(int osc)
{
    return impl_->GetOscNote(osc);
}
const char* Engine::GetChordName()
{
    return impl_->GetChordName();
}
const char* Engine::GetChordNumeral()
{
    return impl_->GetChordNumeral();
}
void Engine::ProcessAudio(float* out, int nframes)
{
    impl_->ProcessAudio(out, nframes);
}
void Engine::RenderScreen(uint16_t* fb)
{
    impl_->RenderScreen(fb);
}
void Engine::RenderDiagnostics(uint16_t* fb)
{
    impl_->RenderDiagnostics(fb);
}
} // namespace ammonite
