...
*/
// Ammonite plugin copy: see reverbsc.h for what changed.
#include "reverbsc.h"
...
int ReverbSc::Init(float sr)
{
    ...
    for(i = 0; i < 8; i++)
    {
        // (the original checked only where a line starts)
        if(n_bytes + DelayLineMaxSamples(sr, 1, i) > AMMONITE_REVERBSC_MAX_SIZE)
            return 1;
